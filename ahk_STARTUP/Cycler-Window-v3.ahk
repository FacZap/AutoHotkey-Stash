...
; -------------------------
#F4::
    total := gWindows.Length()
    if (total = 0)
    {
        TrayTip, No Windows Stored, No windows have been stored yet., 2
        gIndex := 0
        return
    }

    ; NON-destructive cycle: activate the next window that exists RIGHT NOW.
    ; Windows that aren't available (minimized/suspended UWP apps, or momentarily
    ; without a hwnd) are SKIPPED, never deleted -> they can come back later.
    ; Bounded to one full pass: no recursion, no data loss. Closed windows are
    ; pruned manually (Win+Shift+F5) or when opening the list (Win+Shift+F4).
    Loop, % total
    {
        gIndex++
        if (gIndex > total)
    ...
    ; None of the stored windows are available right now (all minimized/closed).
    TrayTip, Cycler, None of the stored windows are available right now., 2
return
...
ShowWindowListGui() {
    ...
    ; vLVWinList now refers to GLOBAL LVWinList var (required in functions)
    Gui, WinList:Add, ListView, w520 r10 Grid -Multi vLVWinList, #|Current|Title|HWND

    ; Make sure LV_* functions target this ListView
    Gui, WinList:Default
    ...
    MouseGetPos, mx, my
    Gui, WinList:Show, x%mx% y%my% NoActivate, WinList
